    ...
    <div className="min-h-screen bg-kooqs-dark">
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="card p-8 text-center mb-6">
          ...
          <h1 className="text-kooqs-text font-black text-2xl">Order Placed! 🎉</h1>
          <p className="text-kooqs-text-dim mt-2">We&apos;ve received your order. We&apos;ll call you on <strong className="text-kooqs-text">{order.phone}</strong> to confirm.</p>
        ...
        {!isCancelled && (
          <div className="card p-6 mb-6">
            <h2 className="text-kooqs-text font-bold mb-5">Order Status</h2>
                  ...
                  return (
                    <div key={step} className="flex flex-col items-center gap-2">
                      <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center z-10 transition-all duration-300 ${
                        ...
                        {done ? <CheckCircle2 size={14} className="text-kooqs-text" /> : <div className="w-2 h-2 rounded-full bg-kooqs-muted" />}
                      </div>
                      <span className={`text-xs font-medium text-center capitalize ${done ? "text-kooqs-text" : "text-kooqs-text-dim"}`}>
                        {step.replace("_", " ")}
                      </span>
                    </div>
                  );
        ...
        {/* Order details */}
        <div className="card p-6 mb-6">
          <h2 className="text-kooqs-text font-bold mb-4">Order Details</h2>
          <div className="space-y-2 text-sm mb-4">
            <div className="flex justify-between">
              <span className="text-kooqs-text-dim">Order type</span>
              <span className="text-kooqs-text">{order.orderType === "delivery" ? "🚗 Delivery" : "🏃 Pickup"}</span>
            </div>
            {order.address && (
              <div className="flex justify-between gap-4">
                <span className="text-kooqs-text-dim flex items-center gap-1"><MapPin size={13} /> Address</span>
                <span className="text-kooqs-text text-right">{order.address}</span>
              </div>
            )}
            <div className="flex justify-between">
              <span className="text-kooqs-text-dim flex items-center gap-1"><Phone size={13} /> Phone</span>
              <span className="text-kooqs-text">{order.phone}</span>
            </div>
          </div>
            ...
            {order.items.map((item) => (
              <div key={item.id} className="flex gap-3 items-center">
                ...
                <div className="flex-1">
                  <p className="text-kooqs-text text-sm font-medium">{item.name}</p>
                  <p className="text-kooqs-text-dim text-xs">x{item.quantity} · {formatPrice(item.price)} each</p>
                </div>
                <span className="text-kooqs-text font-semibold text-sm">{formatPrice(item.price * item.quantity)}</span>
              </div>
            ))}
          </div>
          ...
          <div className="border-t border-kooqs-border pt-4 mt-4 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-kooqs-text">{formatPrice(order.subtotal)}</span>
            </div>
            {order.deliveryFee > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-kooqs-text-dim">Delivery Fee</span>
                <span className="text-kooqs-text">{formatPrice(order.deliveryFee)}</span>
              </div>
            )}
